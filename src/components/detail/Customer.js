...
import {Table, Button, Space, Spin, DatePicker, Form, Input, InputNumber, message} from 'antd'
import { DeleteOutlined, EditOutlined, UserAddOutlined, LoadingOutlined, CheckOutlined, CloseOutlined, PlusOutlined } from '@ant-design/icons'
    ...
    const columns = [
        ...
        { 
            ...
            render: (text, record) => {
                if(record.key === editRowId){
                    return (
                        <Form.Item name="age" rules={[{ type: 'number', min: 1, required: true }]}>
                            <InputNumber min={1}/>
                        </Form.Item>
                    )
                }else{
        ...
        },
        {
            ...
            render: (record) => {
                ...
                if(record.key === editRowId){
                    editButton = ''
                    updateButton =  <Button type="primary" htmlType="submit"><CheckOutlined/></Button>
                    cancelButton =  <Button danger onClick ={() => {setEditRowId(null)}}><CloseOutlined/></Button>
                }
                return(
                    <Space>
                        {editButton}
        ...
        useEffect(() =>{
            form.setFieldsValue({
                ...
                birth: rowData?.dateOfBirth? moment(rowData.dateOfBirth, formatDate) : moment(new Date().toLocaleDateString('en-GB'), formatDate),
                phone: rowData?.phone
            })
        },[rowData])
        ...
        const submitHandler = (values) =>{
            ...
            if(hasNewCustomerBefore){
                ...
                const addCustomer = async () =>{
                    ...
                    const indexRowUpdate = customers.findIndex((item) => item.key === editRowId)
